...
export const MerchantKDS: React.FC<MerchantKDSProps> = ({
  ...
  return (
    <div className="pr-7 xl:pr-9 space-y-3.5 text-xs">
      ...
      <div className="bg-[#fbfbfa] p-3 rounded-[4px] border border-[#e6e6e4] flex items-center justify-between gap-3 flex-wrap shadow-2xs">
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1.5">
            <button
              ...
              className={`px-3 py-1 rounded-full font-medium text-xs flex items-center gap-1.5 cursor-pointer transition-all bg-white border ${
                viewMode === 'grid'
                  ? 'border-zinc-900 text-zinc-900 shadow-2xs font-semibold'
                  : 'border-[#e6e6e4] text-[#787774] hover:text-zinc-900 hover:border-zinc-300'
              }`}
            >
          ...
          <div className="flex items-center gap-1.5">
            {[
              ...
              <button
                ...
                className={`px-3 py-1 rounded-full text-xs font-medium cursor-pointer transition-colors bg-white border ${
                  filterType === f.id
                    ? 'border-zinc-900 text-zinc-900 shadow-2xs font-semibold'
                    : 'border-[#e6e6e4] text-[#5a5854] hover:text-zinc-900 hover:border-zinc-300'
                }`}
              >
                {f.label}
              </button>
        ...
        <div className="flex items-center gap-2.5 flex-wrap">
          ...
          <button
            ...
            className={`px-2.5 py-1 rounded-[2px] font-medium text-xs border flex items-center gap-1.5 transition-colors cursor-pointer shadow-2xs ${
              autoVoiceBroadcast
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          <AnimatePresence>
            {filteredTickets.map((ticket) => {
              ...
              let headerBg = 'bg-[#fbfbfa] text-[#37352f]';
              let timerBadge = 'bg-white text-[#2b593f] border-[#c4dcbc]';

              if (isUrged) {
                headerBg = 'bg-rose-50/70 text-rose-800 border-b border-rose-200';
                timerBadge = 'bg-rose-600 text-white animate-bounce font-medium';
              } else if (isOvertime) {
                headerBg = 'bg-[#fbe4e4]/60 text-[#eb5757]';
                timerBadge = 'bg-[#eb5757] text-white animate-pulse';
              } else if (isWarning) {
                headerBg = 'bg-[#fbf3db]/50 text-[#8f6412]';
              ...
              return (
                <motion.div
                  ...
                  className={`bg-[#fbfbfa] rounded-[4px] border ${
                    isUrged
                      ? 'border-rose-400 shadow-2xs'
                      : isOvertime
                      ? 'border-[#eb5757] shadow-2xs'
                      : 'border-[#e6e6e4]'
                  ...
                  <div>
                    <div className={`p-2.5 flex items-center justify-between border-b border-[#e6e6e4] ${headerBg}`}>
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <span className="font-semibold text-xs text-[#37352f]">{ticket.tableOrChannel}</span>
                        {isUrged && (
                          <span className="text-[10px] font-medium bg-rose-600 text-white px-1.5 py-0.2 rounded-[2px] flex items-center gap-0.5 animate-pulse shadow-2xs">
                            <Flame className="w-2.5 h-2.5 fill-current" />
                            <span>加急催单 {ticket.urgeCount ? `x${ticket.urgeCount}` : ''}</span>
                          </span>
                        )}
                        {/* 取件码 */}
                        <span className="text-[10px] font-mono font-medium bg-amber-50 text-amber-900 border border-amber-200 px-1.5 py-0.2 rounded-[2px] flex items-center gap-0.5 shadow-2xs">
                      ...
                      <div className="flex items-center gap-1.5">
                        <button
                          ...
                          className="p-1 text-[#5a5854] hover:text-[#2b593f] hover:bg-white rounded-[2px] transition-colors cursor-pointer"
                          title="打开在线联络气泡"
                        >
                        ...
                        <button
                          ...
                          className="p-1 text-rose-500 hover:text-rose-700 hover:bg-rose-50 rounded-[2px] transition-colors cursor-pointer"
                          title="作废删除此后厨工单"
                        >
                          <Trash2 className="w-3.5 h-3.5" />
                        </button>
                        <span className={`text-[10px] font-mono font-medium px-1.5 py-0.5 rounded-[2px] border ${timerBadge}`}>
                    ...
                    <div className="p-3 space-y-2">
                      <div className="flex items-center justify-between">
                        <p className="text-[10px] text-[#787774] font-normal">点击菜品划菜 · 制作流转时间线:</p>
                      </div>
                      <div className="space-y-2">
                        {ticket.items.map((it) => {
                          ...
                          return (
                            <div
                              key={it.id}
                              className={`rounded-[2px] border transition-all overflow-hidden ${
                                it.isCompleted
                                  ? 'bg-[#edf3ec]/40 border-[#c4dcbc]'
                                  : 'bg-white border-[#e6e6e4] hover:border-slate-400'
                              }`}
                            >
                              {/* Dish Main Row */}
                              <div className="p-2 flex items-start justify-between gap-2">
                                ...
                                >
                                  <div className="flex items-center gap-1.5 flex-wrap">
                                    <span className={`font-semibold text-xs ${it.isCompleted ? 'text-[#787774] line-through font-normal' : 'text-[#37352f]'}`}>
                                      {it.dishName}
                                    </span>
                                    {it.options && (
                                      <span className="text-[10px] text-[#787774] font-normal truncate">
                                        ({it.options})
                                      </span>
                                    )}
                                  </div>
                                  {it.notes && (
                                    <span className="text-[10px] text-[#d9730d] font-normal block">
                                      ★ 备注: {it.notes}
                                    </span>
                                  )}

                                  {/* Quick summary timing badge */}
                                  <div className="flex items-center gap-2 pt-0.5 text-[10px] font-mono text-slate-500 font-normal">
                                    <span>下单: {timelineData.orderedAt}</span>
                                    {it.isCompleted ? (
                                      <span className="text-emerald-700 font-medium bg-emerald-50 px-1 py-0.2 rounded-[2px] border border-emerald-200">
                                        出餐: {timelineData.servedAt} ({timelineData.totalDurationText})
                                      </span>
                                    ) : (
                                      <span className="text-amber-700 font-normal bg-amber-50 px-1 py-0.2 rounded-[2px] border border-amber-200 flex items-center gap-0.5">
                                ...
                                <div className="flex items-center gap-1.5 shrink-0">
                                  ...
                                  <button
                                    ...
                                    className="p-1 hover:bg-slate-100 rounded-[2px] text-slate-500 hover:text-slate-900 transition-colors cursor-pointer"
                                  ...
                                  >
                                    <span className="font-mono font-semibold text-xs bg-[#fbfbfa] px-1.5 py-0.5 rounded-[2px] border border-[#e6e6e4]">
                                      x{it.quantity}
                                    </span>
                              ...
                              {isTimelineOpen && (
                                <div className="p-2.5 bg-[#fbfbfa] border-t border-[#e6e6e4] space-y-2 text-[11px] animate-in fade-in duration-150">
                                  <div className="flex items-center justify-between border-b border-[#e6e6e4] pb-1">
                                      ...
                                      <div key={nIdx} className="flex items-start gap-2 relative">
                                        ...
                                        <div className="mt-0.5 shrink-0 z-10">
                                          {node.status === 'completed' ? (
                                            <div className="w-3.5 h-3.5 rounded-[2px] bg-emerald-600 text-white flex items-center justify-center shadow-2xs">
                                              <Check className="w-2.5 h-2.5 stroke-[3]" />
                                            </div>
                                          ) : node.status === 'current' ? (
                                            <div className="w-3.5 h-3.5 rounded-[2px] bg-amber-500 text-white flex items-center justify-center shadow-2xs animate-pulse">
                                              <Flame className="w-2.5 h-2.5" />
                                            </div>
                                          ) : (
                                            <div className="w-3.5 h-3.5 rounded-[2px] bg-slate-200 text-slate-500 flex items-center justify-center">
                                        ...
                                        <div className="flex-1 leading-tight">
                                          <div className="flex items-center justify-between font-normal">
                                            <span className={`text-xs ${node.status === 'completed' ? 'text-slate-900 font-medium' : node.status === 'current' ? 'text-amber-800 font-medium' : 'text-slate-400'}`}>
                                              {node.label}
                                            </span>
                  ...
                  <div className="p-2 bg-[#fbfbfa] border-t border-[#e6e6e4] flex flex-wrap sm:flex-nowrap items-center gap-1.5">
                    <div className="flex items-center gap-1.5 shrink-0">
                      ...
                      <button
                        ...
                        className="py-1.5 px-2 bg-white hover:bg-amber-50 text-amber-900 rounded-[2px] font-medium text-xs flex items-center justify-center gap-1 cursor-pointer transition-colors border border-amber-200 shadow-2xs"
                      ...
                      <button
                        ...
                        className="py-1.5 px-2 bg-white hover:bg-rose-50 text-rose-700 rounded-[2px] font-medium text-xs flex items-center justify-center gap-1 cursor-pointer transition-colors border border-rose-200 shadow-2xs"
                        title="撤单作废此后厨工单"
                      >
                    ...
                    <div className="flex items-center gap-1.5 flex-1 min-w-[140px]">
                      ...
                      <button
                        ...
                        className={`flex-1 py-1.5 rounded-[2px] font-medium text-xs flex items-center justify-center gap-1.5 cursor-pointer transition-all shadow-xs ${
                          allItemsCompleted
                      ...
                      <button
                        ...
                        className="py-1.5 px-2 bg-white hover:bg-[#f1f1ef] text-neutral-700 rounded-[2px] font-medium text-[11px] cursor-pointer transition-colors border border-[#e6e6e4] shrink-0"
      ...
      {viewMode === 'batch' && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {Object.entries(dishAggregates).map(([dishName, data]) => (
            <div
              key={dishName}
              className="bg-[#fbfbfa] rounded-[4px] border border-[#e6e6e4] p-3.5 space-y-3 shadow-2xs flex flex-col justify-between"
            >
              <div>
                <div className="flex items-start justify-between gap-2 border-b border-[#e6e6e4] pb-2">
                  <div>
                    <h4 className="font-semibold text-sm text-[#37352f]">{dishName}</h4>
                    <span className="text-[10px] text-[#787774] font-normal">跨单批量翻炒制作</span>
                  </div>
                  <span className="font-mono font-medium text-sm bg-[#d9730d] text-white px-2 py-0.5 rounded-[2px]">
                    共需 {data.totalQty} 份
                  </span>
                </div>
                ...
                <div className="pt-2 space-y-1.5">
                  <span className="text-[10px] text-[#787774] font-normal block">分布订单与桌位:</span>
                  <div className="space-y-1">
                    {data.ticketDetails.map((td, idx) => (
                      <div key={idx} className="flex items-center justify-between bg-white p-1.5 rounded-[2px] border border-[#e6e6e4] text-[11px]">
                        <span className="font-mono text-[#37352f]">{td.ticketNo} · {td.table}</span>
                        <div className="flex items-center gap-1.5">
                          <span className="font-mono font-semibold text-[#d9730d]">x{td.qty}份</span>
                          <button
                            type="button"
                            onClick={() => {
                  ...
                  {data.specialNotes.length > 0 && (
                    <div className="bg-[#fbf3db]/50 p-2 rounded-[2px] border border-[#ecd9a8] text-[10px] text-[#8f6412] space-y-0.5">
                      <span className="font-medium">特殊要求汇总:</span>
              ...
              <div className="flex gap-1.5 pt-2">
                <button
                  ...
                  className="flex-1 py-1.5 bg-[#37352f] hover:bg-[#201f1d] text-white rounded-[2px] font-medium text-xs flex items-center justify-center gap-1 cursor-pointer transition-all shadow-2xs"
                  title="仅标记出锅划菜"
                >
                ...
                <button
                  ...
                  className="px-3 py-1.5 bg-emerald-700 hover:bg-emerald-800 text-white rounded-[2px] font-medium text-xs flex items-center justify-center gap-1 cursor-pointer transition-all shadow-xs"
      ...
      {deleteTargetTicket && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-3">
          <div className="bg-[#fbfbfa] w-full max-w-md rounded-[4px] border border-[#e6e6e4] shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-150">
            {/* Header */}
            <div className="p-3 bg-[#37352f] text-white flex items-center justify-between">
            ...
            <div className="p-4 space-y-3 text-xs">
              <div className="p-2.5 bg-rose-50 border border-rose-200 text-rose-800 space-y-1 rounded-[2px]">
              ...
              {/* Ticket details */}
              <div className="bg-white p-3 border border-[#e6e6e4] rounded-[2px] space-y-2">
                <div className="flex items-center justify-between">
                ...
                <div className="text-[11px] text-slate-700 pt-1 border-t border-dashed border-[#e6e6e4]">
                  餐台/渠道: <strong className="text-slate-900 font-semibold">{deleteTargetTicket.tableOrChannel}</strong>
                </div>

                <div className="space-y-1 pt-1">
                  <span className="text-[10px] text-slate-500 font-normal block">待制菜品:</span>
                  <div className="space-y-1 max-h-28 overflow-y-auto">
                    {deleteTargetTicket.items.map((it) => (
                      <div key={it.id} className="flex justify-between text-[11px] bg-[#fbfbfa] p-1.5 border border-[#e6e6e4] rounded-[2px]">
                        <span className={it.isCompleted ? 'line-through text-slate-400 font-normal' : 'text-slate-800 font-normal'}>
                          {it.dishName}
                        </span>
                        <span className="font-mono font-semibold text-amber-700">x{it.quantity}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="space-y-1.5">
                <label className="font-medium text-slate-800 block">选择作废原因:</label>
                <select
                  value={deleteReason}
                  onChange={(e) => setDeleteReason(e.target.value)}
                  className="w-full p-2 bg-white border border-[#e6e6e4] rounded-[2px] focus:outline-none focus:border-[#37352f] text-xs font-normal"
                >
                ...
                {deleteReason === 'other' && (
                  <input
                    ...
                    className="w-full p-2 mt-1.5 bg-white border border-[#e6e6e4] rounded-[2px] focus:outline-none focus:border-[#37352f] text-xs"
                    autoFocus
                  />
                )}
              </div>
              ...
              <label className="flex items-center gap-2 text-slate-700 cursor-pointer select-none text-[11px] pt-1">
                <input
                  ...
                  className="rounded-[2px] text-rose-600 focus:ring-0"
                />
                <span className="font-medium text-slate-800">同步在全渠道订单中心彻底删除该笔订单（连带作废）</span>
              </label>
            </div>

            {/* Footer */}
            <div className="p-3 bg-white border-t border-[#e6e6e4] flex items-center justify-end gap-2">
              <button
                type="button"
                onClick={() => {
                  setDeleteTargetTicket(null);
                ...
                className="px-3.5 py-1.5 bg-white hover:bg-[#f1f1ef] text-slate-700 border border-[#e6e6e4] rounded-[2px] font-medium cursor-pointer text-xs"
              >
                取消
              </button>
              <button
                ...
                className="px-4 py-1.5 bg-rose-600 hover:bg-rose-700 text-white rounded-[2px] font-medium cursor-pointer text-xs flex items-center gap-1.5 shadow-xs"
              >
                <Trash2 className="w-3.5 h-3.5" />
                <span>确认删除工单</span>
              </button>
